...
- **Step 2** — Explore. **Use the code-intelligence layer first** (`shared.md` Step 2.0): if `graphify-out/graph.json` / `GRAPH_REPORT.md` exist, read them to seed the model; if the `gitnexus` CLI is available and the repo is indexed, use `gitnexus query`/`context` (via Bash). Also run `ca-tools graph review-prep` once to get the changed files + a deterministic blast-radius table for Step 3. Fall back to Glob/Grep for gaps. You **cannot** call the Codeflow MCP server or spawn the `code-intel` agent — a subagent has no MCP/Agent access; use the CLIs and `graph.json` directly. Never run a long `gitnexus analyze` yourself — note it and fall back. For repos > 100 files the graph is the primary tool.
- **Step 3** — Perform the review. Use the variant file's anti-pattern checklist explicitly. **Ground severity in measured blast radius** (`shared.md` Step 3): use the `graph review-prep` blast-radius table (`symbol → impactedCount, risk`) to justify Severity/Priority — a high-fan-in symbol is higher severity, cited with evidence. Ground every observation in `file:line`. If `review-prep` reports `available: false`, score by judgment and note blast radius was not measured.
